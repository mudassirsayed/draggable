        ...
        update: function (ev, ui) {
            ...
            if (window.localStorage.getItem("state") == "1") {
                ...
                if (from != to) {
                    ...
                    store_new_item(from, id, document.getElementById(id).innerHTML);
                }
                reorder_lists(to, $(this).sortable('toArray'));
            } else {
                window.localStorage.setItem("state", "1");
            }
...
function load_items() {
    var box1_default_items = { "b1_1": "box 1 item 1", "b1_2": "box 1 item 2", "b1_3": "box 1 item 3" };
    var box2_default_items = { "b2_1": "box 2 item 1", "b2_2": "box 2 item 2", "b2_3": "box 2 item 3" };
...
function store_new_item(box_id, id, val) {
    if (box_id == "box1_list") {
        var box1_items = JSON.parse(window.localStorage.getItem("vals1"));
        box1_items["nb1_" + id] = val;
        $("#box1_list").append("<li id=nb1_" + id + ">" + val + "</li>");
        window.localStorage.setItem("vals1", JSON.stringify(box1_items));
    }
    else if (box_id == "box2_list") {
        var box2_items = JSON.parse(window.localStorage.getItem("vals2"));
        box2_items["nb2_" + id] = val;
        $("#box2_list").append("<li id=nb2_" + id + ">" + val + "</li>");
        window.localStorage.setItem("vals2", JSON.stringify(box2_items));
